 ...
 *         <p>
 *         Problem: CNOTE     Easy          Chef and Notebooks    .
 */

public class ChefAndNotebooks {
    public ChefAndNotebooks() {
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            int[] data = scanner.nextIntArray(4);

            int p_needed = data[0] - data[1];
            int money = data[2];

            for (int j = 0; j < data[3]; j++) {
                int[] data2 = scanner.nextIntArray(2);

                if (data2[0] >= p_needed && data2[1] <= money) {
                    success = true;
                    // Do not break in order to read all the data from the input.
                    //   break;
                }
            }
