 ...
 *         <p>
 *         Problem: CHEFSTLT   Beginner      Chef and Two Strings  .
 */

public class ChefAndTwoStrings {
    public ChefAndTwoStrings() {
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < string.length(); j++) {
                if (string.charAt(j) == '?' || string2.charAt(j) == '?') {
                    max++;
                } else if (string.charAt(j) != string2.charAt(j)) {
                    min++;
                    max++;
                }
            }
            out.format("%d %d %n", min, max);
        }
