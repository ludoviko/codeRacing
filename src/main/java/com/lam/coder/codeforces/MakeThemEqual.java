...
public class MakeThemEqual {
	...
	public static void main(String[] args) throws IOException {
        ...
        Integer[] data = scanner.nextIntegerArray(n);
        Integer[] sorted  = null;

        Set<Integer> set = new HashSet<>();
        set.addAll(Arrays.asList(data));

        sorted = set.toArray(new Integer[set.size()]);
        ...
        if (sorted.length > 3) {
            ...
            if (sorted[2] - sorted[1] == sorted[1] - sorted[0]) {
                System.out.println(sorted[2] - sorted[1]);
            } else {
                System.out.println(-1);
            }
        } else if (sorted.length == 2) {
            if ( (sorted[1] - sorted[0]) % 2 ==  0) {
                System.out.println((sorted[1] - sorted[0]) / 2 );
            } else {
                System.out.println((sorted[1] - sorted[0]));
            }
