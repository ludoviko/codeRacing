 ...
 *
 * Problem: 294A  #178 Div-2  Shaass and Oskols.
 *
 *
 */
...
public class ShaassAndOskols {
	...
	public static void change(int[] birds, int wire, int positionShot) {
        int index  = wire - 1;

        if (index == 0) {
            if (birds.length > 1) {
                birds[index + 1] += birds[index] - positionShot;
            }
        } else if (index == birds.length - 1) {
            birds[index - 1] += positionShot - 1;
        } else {
            birds[index + 1] += birds[index] - positionShot;
            birds[index - 1] += positionShot - 1;
        }
	...
	public static void main(String[] args) throws IOException {
        ...
        int[] birds = scanner.nextIntArray(wires);
