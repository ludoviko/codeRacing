 ...
 * @author LAzuaje
 * 
 *         A. Way Too Long Words time limit per test2 seconds memory limit per
 *         test256 megabytes inputstandard input outputstandard output Sometimes
 *         some words like "localization" or "internationalization" are so long
 ...
 *         between the first and the last letters. That number is in decimal
 ...
 *         characters.
 * 
 *         Output Print n lines. The i-th line should contain the result of
 *         replacing of the i-th word from the input data.
 * 
 *         Sample test(s) input 4 word localization internationalization
 *         pneumonoultramicroscopicsilicovolcanoconiosis output word l10n i18n
 *         p43s
 * 
 *
 */
...
public class WayTooLongWords {
	public String find(String string) {
		...
		data += string.charAt(0) + "" + (len - 2) + "" + string.charAt(len - 1);
	...
	public static void main(String[] args) throws IOException {
		...
		int i = 0;
		while (i < n) {
			System.out.println(words.find(scanner.next()));
			i++;
		}
