 ...
 *         Assume that:
 * 
 *         N is an integer within the range [0..1,000,000]; each element of
 *         array A is an integer within the range
...
// XXX re-write using stack and the convenient leader algorithm

public class Dominator {

	public int solution(long... array) {
		int beyondHalf = array.length / 2 + 1;

		if (array.length == 0) {
			return -1;
		}
		
		Map<Long, Long> histogram = Histogram.findHistogram(array);
		Entry<Long, Long> entry = Histogram.findModa(histogram);

		Long key = entry.getKey();
		Long counter = entry.getValue();

		if (counter >= beyondHalf) {
	...
	public static void main(String[] args) {
		...
		System.out.println(dominator.solution(4, 3, 3, 2, 3, -1, 3, 3));
		System.out.println(dominator.solution(3, 4, 3, 2, 3, -1, 3, 7));
		System.out.println(dominator.solution(-3, 4, -3, 2, -3, -1, -3, -3));
		System.out.println(dominator.solution());
