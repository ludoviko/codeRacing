 ...
 *
 *         increase(X) − counter X is increased by 1, max_counter − all counters
 *         are set to the maximum value of any counter.
 *
 ...
 *         A[K] = N + 1 then operation K is max_counter.
 *
 ...
 *
 *         a structure Results (in C), or a vector of integers (in C++), or a
 *         record Results (in Pascal), or an array of integers (in any other
...
public class MaxCounters {
	public int[] solution(int n, int[] commands) {
		...
		for (int i = 0; i < commands.length; i++) {
			if (commands[i] <= n) {
				counters[commands[i] - 1] = Math.max(counters[commands[i] - 1],
						last);

				counters[commands[i] - 1]++;
				max = Math.max(counters[commands[i] - 1], max);
	...
	public int[] solutionOld(int n, int[] commands) {
		int[] counters = new int[n];
		int max = 0;
		for (int i = 0; i < commands.length; i++) {
			if (commands[i] <= n) {
				counters[commands[i] - 1]++;
				if (counters[commands[i] - 1] > max) {
					max = counters[commands[i] - 1];
				}
			} else {
