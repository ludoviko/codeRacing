...
public class BinaryGap {
	...
	public int solution(int n) {
		...
		int[] prefix_0 = ArrayUtil.findPrefixSum(0, array);
		int[] prefix_1 = ArrayUtil.findPrefixSum(1, array);

		for (int i = 1; i < prefix_0.length; i++) {
			if (prefix_1[i - 1] != prefix_1[i]) {
				if (count == 2) {
					sum = prefix_0[i] - prefix_0[start];
					count = 1;
					start = i;
					if (sum > max) {
						max = sum;
