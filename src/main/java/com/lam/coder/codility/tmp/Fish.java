 ...
 *         ordered downstream along the flow of the river. The fish are numbered
 *         from 0 to N − 1, fish number P is represented by A[P] and B[P], and
 *         if P < Q then fish P is initially upstream of fish Q. Initially, each
 *         fish has a unique position.
 * 
 *         Array A contains the sizes of the fish. All its elements are unique.
 *         Array B contains the directions of the fish. It contains only 0s
 *         and/or 1s, where:
 * 
 ...
 *         one. More precisely, we say that two fish P and Q meet each other
 *         when P < Q, B[P] = 1 and B[Q] = 0, and there are no living fish between them. After they meet: If A[P] >
 *         A[Q] then P eats Q, and P will still be flowing downstream, If A[Q] >
 ...
 *         4 and is eaten by it. The remaining two fish, numbers 0 and 4, never
...
public class Fish {
	public int solution(int[] sizes, int[] fishes) {

		Stack<Integer> stack = new Stack<Integer>();
		int summation = fishes.length;

		int aux = 0;

		for (int i = 0; i < fishes.length - 1; i++) {
			if (stack.isEmpty()) {
				if (fishes[i] == 1 && fishes[i + 1] == 0) {
			...
			} else if (stack.peek() == 1 && fishes[i + 1] == 0) {
				if (sizes[aux] > sizes[i + 1]) {
					sizes[i + 1] = 0;
				} else {
					stack.pop();
					aux = 0;
