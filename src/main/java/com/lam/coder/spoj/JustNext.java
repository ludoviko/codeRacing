...
//	}
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < cases; i++) {
            ...
            source = Arrays.toString(data).replaceAll("\\[|\\]|,|\\s", "");
            ...
            for (int j = size - 1; j >= 1; j--) {
                if (data[j] > data[j - 1]) {
                    anyChange = true;
                    int x = data[j - 1];
                    // Find the smallest number from the right of x that is greater than x.
                    for (int k = size - 1; k >= j; k--) {
                        if (x < data[k]) {
                            int t = data[k];
                            data[k] = x;
                            data[j - 1] = t;
                            // Sort elements fron righg of (j-1)
                            Arrays.sort(data, j, size);
                            break;
                        } else {
                        }
                    }
            ...
            String solution = Arrays.toString(data).replaceAll("\\[|\\]|,|\\s", "");
            if (solution.equals(source)) {
                out.println(-1);
            } else {
