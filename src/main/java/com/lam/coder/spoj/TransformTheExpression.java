...
public class TransformTheExpression {
    ...
    public static void main(String[] args) throws IOException {
        ...
        TransformTheExpression main = new TransformTheExpression();

        int n = scanner.nextInt();
        Set<Character> validOperators = new HashSet<>();
        validOperators.addAll(Arrays.asList('+', '-', '*', '/', '^'));
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < string.length(); j++) {
                ...
                if (Character.isLetter(next)) {
                    ...
                    builder.append(next);
                } else if (next == openingBracket) {
                } else if (next == closingBracket) {
                    builder.append(operators.removeFirst());
                } else {
                    operators.addFirst(next);
                }
            }
