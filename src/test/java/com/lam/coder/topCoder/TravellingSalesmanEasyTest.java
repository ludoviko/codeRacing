...
import com.lam.coder.topCoder.TravellingSalesmanEasy;
...
public class TravellingSalesmanEasyTest {
	TravellingSalesmanEasy travellingSalesmanEasy;
	
	@Before
	public void setUp() throws Exception {
		travellingSalesmanEasy = new TravellingSalesmanEasy();
	}

	@After
	public void tearDown() throws Exception {
		travellingSalesmanEasy  = null;
	}

	@Test
	public void test() {
        Assert.assertThat("Should be the same.", travellingSalesmanEasy.getMaxProfit(2, new int[] {3, 10}, new int[] {1, 1}, new int[] {2, 1}), CoreMatchers.is(10));
        Assert.assertThat("Should be the same.", travellingSalesmanEasy.getMaxProfit(1, new int[] {3,5,2,6,4}, new int[] {1,1,1,1,1}, new int[] {1,1,1}), CoreMatchers.is(15));
        Assert.assertThat("Should be the same.", travellingSalesmanEasy.getMaxProfit(6, new int[] {77,33,10,68,71,50,89}, new int[] {4,1,5,6,2,2,1}, new int[] {6,5,5,6,4}), CoreMatchers.is(155));

        Assert.assertThat("Should be the same.", travellingSalesmanEasy.getMaxProfit(7, new int[] {22,91,53,7,80,100,36,65,92,93,19,92,95,3,60,50,39,36,2,30,63,84,2},
                new int[] {5,3,4,3,6,5,6,6,5,2,7,6,3,2,6,1,7,4,6,3,7,2,5}, new int[] {5,7,1,3,6,2,5,7,3,6,3,2,7,3,1,3,1,7,2,3,1,1,3,1,6,1}), CoreMatchers.is(1003));

        Assert.assertThat("Should be the same.", travellingSalesmanEasy.getMaxProfit(85, new int[] {94,21,99,27,91,1,64,96,32,39,84,71,97,53,73,20,7,13,33,45,5,85,7,87,
      ...
      //  Assert.assertThat("Should be the same.", travellingSalesmanEasy.getMaxProfit(0, new int[] {}, new int[] {}, new int[] {}), CoreMatchers.is(0));
	}
}
